      ...
      cy.get('@testData').then((testData) => {
        testData.split('\n').forEach((line, index) => {
          if (index > 0) { // skip header row
            ...
            cy.getPropertyTransacIndividual(propertyID).then((response) => {
              ...
              if (response.body.length === 0) {
                ...
                cy.wrap(response.body).each((obj) => {
                  ...
                  if (obj.hasOwnProperty('registryUniId')) {
                    const value = obj.registryUniId;
                    try {
                        expect(value).to.not.be.null;
                        expect('number').to.include(typeof value);
  
                    } catch (error) {
                         cy.log(`Assertion failed for Property ID: ${propertyID}, Field: registryUniId - ${error.message}`);
                  ...
                  if (obj.hasOwnProperty('transactionPrice')) {
                    const value = obj.transactionPrice;
                    try {
                        expect(value).not.to.be.null;
                        expect('number').to.include(typeof value);
  
                    } catch (error) {
                         cy.log(`Assertion failed for Property ID: ${propertyID}, Field: transactionPrice - ${error.message}`);
                  ...
                  if (obj.hasOwnProperty('personId')) {
                    const value = obj.personId;
                    try {
                        expect(value).not.to.be.null;
                        expect('number').to.include(typeof value);
  
                    } catch (error) {
                         cy.log(`Assertion failed for Property ID: ${propertyID}, Field: personId - ${error.message}`);
                  ...
                  if (obj.hasOwnProperty('type')) {
                    ...
                    try {
                        ...
                        expect('string').to.include(typeof value);
                         ...
                         cy.log(`Assertion failed for Property ID: ${propertyID}, Field: type - ${error.message}`);
