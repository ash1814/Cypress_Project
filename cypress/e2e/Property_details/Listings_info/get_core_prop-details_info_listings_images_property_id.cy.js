      ...
      cy.get('@testData').then((testData) => {
        testData.split('\n').forEach((line, index) => {
          if (index > 0) { // skip header row
            ...
            cy.getPropertyInfoListingImages(propertyID).then((response) => {
                  ...
                  if (obj.hasOwnProperty('images')) {
                    ...
                    try {
                      ...
                      value.forEach((item) => {
                        ...
                        // Additional assertions for the properties of the object if needed
                      });
                    } catch (error) {
                       cy.log(`Assertion failed for Property ID: ${propertyID}, Field: images - ${error.message}`);
                    }
                  }
                });
              }
